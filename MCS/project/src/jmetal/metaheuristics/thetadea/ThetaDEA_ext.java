...
import jmetal.util.ranking.ThetaRanking_ext;
import jmetal.util.Stats;


public class ThetaDEA_ext extends Algorithm {
	...
	public ThetaDEA_ext(Problem problem) {
		super(problem);
	} // ThetaDEA 
...
/* 			if(generations_ == 1) {
	...
	void getNextPopulation(SolutionSet pop){
		Ranking ranking = new ThetaRanking_ext(pop, lambda_, zideal_, theta_, normalize_);
	...
	void doCrossover(int i) throws JMException{
		...
		// If problem_ defines evaluateConstraints, this should set offSpring[0].overallConstraintViolation
	...
	void normalizePopulation(SolutionSet pop) {
		...
		double valmin = (pop.get(0).getObjective(0) - zideal_[0]) / (znadir_[0] - zideal_[0]);
		...
		for (int i = 0; i < pop.size(); i++) {
			...
			for (int j = 0; j < obj; j++) {
				double val = (sol.getObjective(j) - zideal_[j]) / (znadir_[j] - zideal_[j]);
		...
		double delta = valmax == valmin ? 1 : valmax - valmin;
		for (int i = 0; i < pop.size(); i++) {
			Solution sol = pop.get(i);
			double cv = Math.abs(sol.getOverallConstraintViolation());
			double ncv = (cv - valmin) / delta;
			sol.setNormalizedConstraintViolation(ncv);
		}

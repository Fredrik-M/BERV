...
public class ThetaRanking_ext implements Ranking {
	...
	public ThetaRanking_ext(SolutionSet solutionSet,  double[][] lambda, 
			double[] zideal, 
	...
	double[] getDistancesWithNormalize(Solution sol, double[] ref) {
		...
		double[] d = new double[2];
		// d1 (norm of projection of f on lambda)
		for (int j = 0; j < obj_; j++) {
		...
		d[1] = 0;
		int m = sol.numberOfObjectives();
		// d2 (orthogonal distance between f and lambda)
		for (int i = 0; i < m; i++) {
			double x = sol.getNormalizedObjective(i);
			double ncv = sol.getNormalizedConstraintViolation();
			d[1] += Math.pow(x + (x * ncv / m) - d[0] * (ref[i] / refLenSQ), 2);
		}

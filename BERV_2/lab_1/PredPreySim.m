...
% Initial conditions, e.g. the mean density of predator and prey in a 
% certain area at time zero
pred0 = 500;
prey0 = 1000;
...
setting = odeset('OutputFcn','odeplot','RelTol', 1e-3);
% The relative tolerance will be changed to 10^-2 and the ode-solver will
% plot the solution automatically

% Call ode45. The parametern setting is optional. Used only when changing default settings. 
...
if isempty(setting.OutputFcn)
  ...
  xlabel('time')
  ylabel('Mean density of predator and prey');
  title('Predator, prey simulation');
  legend('Mean density of prey','Mean density of predator');